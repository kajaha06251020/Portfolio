...
Python (ゲームエンジン)
  ...
  │     ├── npc.say(speaker, text)        -- 対話表示（コルーチンyield）
  │     ├── npc.choice(options)           -- 選択肢表示→結果返却（コルーチンyield）
  │     │
  │     ├── quest.start(quest_id)         -- クエスト開始（inactive→active）
  │     ├── quest.complete()              -- 現コンテキストのクエスト完了
  │     ├── quest.get_state(quest_id)     -- クエスト状態取得（"inactive"/"active"/"completed"等）
  │     ├── quest.get_stage()             -- 現コンテキストのクエストのステージ取得
  │     ├── quest.set_stage(stage_name)   -- 現コンテキストのステージ設定
  │     ├── quest.set_objective(text)     -- 現コンテキストの目標テキスト設定
  │     ├── quest.update(quest_id, stage) -- 任意クエストのステージを直接設定
  │     │
  │     ├── world.get_layer()             -- 現在の層を返す ("physical"/"depth"/"dream")
  ...
  │     │
  │     ├── event.trigger(event_id)       -- イベント発火
  │     └── rules.register(event_type, callback)  -- ルールコールバック登録
  │
...
### 2.3 Quest API コンテキストモデル

クエスト系APIには「明示的」と「コンテキスト依存」の2種類がある。

- **明示的:** `quest.get_state(quest_id)`, `quest.start(quest_id)`, `quest.update(quest_id, stage)` — 任意のクエストを指定して操作
...
NPCスクリプトが実装できるコールバック一覧:

| コールバック | 呼び出しタイミング |
|-------------|-------------------|
| `on_talk()` | プレイヤーがNPCに話しかけた時 |
| `on_visible()` | マップ描画前にNPC表示可否を判定（`true`/`false`を返す） |
...
// data/npcs.json
{
  "npc_001": {
    ...
    "layers": {
      ...
      "depth":    { "map": "depth_ruins", "x": 5, "y": 3, "script": "npc/alma_depth.lua" },
      "dream":    null
    },
    "script": "npc/alma.lua",
    "movement": "fixed"
...
- `layers`: 三層それぞれでの配置。`null`はその層に存在しない
- `layers[layer].script`: 層固有のスクリプト（省略時はトップレベルの`script`を使用、内部で`world.get_layer()`分岐）。層間で挙動が大きく異なるNPCに使用
- `movement`: 移動パターン。`"fixed"` / `"patrol"` / `"random"` から選択
...
| 要素 | 仕様 |
|------|------|
| 対話キー | Enter / Space / Z キー |
| 対話範囲 | プレイヤーが向いている方向の隣接1タイル |
| 複数NPC | プレイヤーの向き方向にあるNPCを優先。同一タイルに複数いる場合はJSON定義順で最初のNPC |
| 対話中の制限 | プレイヤー移動不可、エンカウント判定停止 |
| 対話キャンセル | 選択肢表示中以外は Escape キーで対話を中断可能 |

既存の `MapScene` にインタラクションキーのハンドリングを追加する。
...
| 移動パターン | `fixed`（固定）、`patrol`（巡回）、`random`（ランダム歩行）— NPCごとにJSON設定 |
...
function on_stage_event(trigger)
  ...
  elseif stage == "report" and trigger == "talk_alma" then
    ...
    if choice == 1 then
      ...
      world.set_state("depth.distortion",
        world.get_state("depth.distortion") - 2)
...
| 敵AIが通常攻撃のみ | `enemies.json` に既にアビリティリストが定義済み。`_enemy_act()` にAIルーチンを実装し、既存のアビリティリストから選択させる |
...
| `data/enemies.json` | 敵AIルーチン用のアクション重み・条件を追加（アビリティリストは既存） |
...
### 8.2 エラーハンドリングとタイムアウト

| シナリオ | 対応 |
|---------|------|
| **Luaスクリプトが例外を投げる** | `pcall` でラップして例外をキャッチ。対話ウィンドウを閉じ、エラーをログに記録。ゲームはマップシーンに復帰 |
| **無限ループ（暴走スクリプト）** | lupaの命令カウント制限（instruction count hook）を使用。1回のコルーチン実行で最大100,000命令。超過時はコルーチンを強制終了し、対話ウィンドウを閉じる |
| **プレイヤーがEscapeで対話中断** | `npc.say()` のyield中にEscapeが押された場合、コルーチンを破棄して対話ウィンドウを閉じる。`npc.choice()` 表示中はEscapeを無視（選択を強制） |
| **シーン遷移** | 対話中のシーン遷移は禁止。コルーチンが完了または中断されるまでシーン遷移をブロックする |

### 8.3 コルーチン状態とセーブの関係

コルーチンの実行状態はシリアライズ不可。セーブ/ロード実装時は以下のルールに従う:

- **対話中のセーブは禁止** — メニューを開けないため自然に防がれる
- **クエストの進行状態はフラグ/ステージで永続化** — コルーチン内の一時変数には依存しない設計とする
- `npc.say()` の途中で中断されても、フラグが未設定なら次回話しかけた時に同じ分岐を通る

---

## 9. キャラクターロースター

現在のコードベースではFF5のプレースホルダー名（バッツ/レナ/ガラフ）を使用している。回想物語聖典では独自キャスト（ヴェル/カルト/レイン/ブリューン/サーシャ）が定義されている。

**方針:** NPC/クエストシステムは `party.has_member(name)` 等でキャラクター名を参照する。本仕様のシステム実装段階ではプレースホルダー名で開発し、ストーリー実装フェーズで一括置換する。

**移行時の注意:**
- Luaスクリプト内のキャラクター名参照はすべて定数化する（`scripts/common/characters.lua` に名前定義を集約）
- `CharacterDataManager` のキャラクター定義を回想物語聖典のロースターに差し替える
- パーティ構成が3人→5人に拡大する可能性があるため、バトルシーンのレイアウトは4人以上にも対応できる設計とする
...
セーブ/ロードは別途設計するが、本仕様のシステムは以下のシリアライズ要件を満たすこと:

- `WorldStateManager` の全状態（`global`, `layers`, `flags`, `quest_states`）はJSON形式でシリアライズ可能であること
- Luaコールバック参照や関数オブジェクトを状態オブジェクトに格納しないこと
- クエストの進行状態はすべて `quest_states` 辞書内の文字列値で表現すること
- NPCの一時的な対話状態（コルーチン）はシリアライズ対象外
